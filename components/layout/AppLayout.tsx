...
export default function AppLayout({ children }: Props) {
  ...
  if (isPublic) {
    return (
      <div className="min-h-screen bg-paper">
        <div className="max-w-sm mx-auto min-h-screen bg-paper">
          {children}
        </div>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-paper" style={{ fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      <div className="max-w-sm md:max-w-6xl mx-auto min-h-screen bg-paper flex flex-col md:flex-row">

        {/* Desktop sidebar */}
        <aside className="hidden md:flex flex-col w-52 flex-shrink-0 border-r border-line min-h-screen sticky top-0 h-screen">
          ...
          <div className="flex items-center gap-2.5 px-5 py-5 border-b border-line">
            ...
            <div>
              <p className="text-body-sm font-medium text-ink leading-none">
                Pulse<em style={{ fontFamily: "'Fraunces', serif" }} className="italic font-light">link</em>
              </p>
              <p className="label-caps text-ink-3 mt-0.5 capitalize">{effectiveRole}</p>
            </div>
          </div>
            ...
            {config.nav.map(item => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-body-sm transition-all text-left w-full ${
                    active
          ...
          <div className="p-3 border-t border-line">
            <div className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl bg-paper-2">
              <div className={`w-7 h-7 rounded-full ${config.avatarBg} flex items-center justify-center flex-shrink-0`}>
                <span className="text-label font-medium text-paper">
                  {user?.name?.charAt(0)?.toUpperCase() ?? 'U'}
                </span>
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-body-sm font-medium text-ink truncate">{user?.name}</p>
                <p className="label-caps text-ink-3 capitalize">{user?.actualRole}</p>
              </div>
            </div>
          </div>
        </aside>
        ...
        {/* Mobile bottom nav */}
        <MobileNav items={config.nav} activeColor={config.activeColor} />
